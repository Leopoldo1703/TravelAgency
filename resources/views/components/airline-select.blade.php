<div class="relative">
    ...
    <button id="airline-dropdown-btn" type="button" class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 border border-gray-300 focus:outline-indigo-600 sm:text-sm/6">
    ...
    <ul id="airline-list" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base ring-1 shadow-lg ring-black/5 hidden sm:text-sm">
        <!-- Aquí se llenarán las aerolíneas dinámicamente -->
    </ul>
</div>
...
<script>
    ...
    async function loadAirlines() {
        try {
            ...
            dropdownList.innerHTML = ""; // Limpiar antes de insertar
